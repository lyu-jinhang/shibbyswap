...
const WinningNumbers: React.FC = () => {
  ...
  /* eslint-disable */
  const exportRecent = async (event) => {
    event.preventDefault()
    const data = await lottery(25)
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(data));
    const downloadAnchorNode = document.createElement('a');
    downloadAnchorNode.setAttribute("href",     dataStr);
    downloadAnchorNode.setAttribute("download", "RecentWinningNumbers.json");
      ...
      <Card>
        <CardBody>
          <StyledCardContentInner>
            ...
            <Link onClick={exportRecent} href="#" target="_blank" >
...
const TicketNumberBox = styled.div`
  padding: 10px;
  border-radius: 12px;
  background: linear-gradient(180deg, #dc54de 0%, #7924d6 76.22%);
  color: white;
  font-size: 20px;
  font-weight: 900;
  margin: 10px;
